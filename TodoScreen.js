...
const TodoScreen = () => {
  ...
  const deleteTodo = (id) => {
    const updatedTodos = todos.filter((todo) => todo.id !== id); // Lọc công việc không có id tương ứng
    setTodos(updatedTodos); // Cập nhật danh sách công việc
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={todos}
        keyExtractor={(item) => item.id.toString()} // Khóa cho từng mục
        renderItem={({ item }) => (
          <View style={styles.todoItem}>
            <TouchableOpacity onPress={() => toggleTodoComplete(item.id)}>
              <Text style={[styles.todoText, item.completed && styles.completedTodo]}>
                {item.text} {/* Hiển thị nội dung công việc */}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => deleteTodo(item.id)}>
              <Text style={styles.deleteButton}>X</Text> {/* Nút xóa công việc */}
            </TouchableOpacity>
          </View>
        )}
      />
...
export default TodoScreen; // Xuất thành phần TodoScreen
